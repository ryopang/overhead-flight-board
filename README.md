...
- **Synthesized mechanical sound** — no audio files. Each flip's clack is built at runtime from a layered Web Audio graph: a noise transient, a metallic tick (bank of inharmonic square oscillators, the same technique used for drum-machine hi-hats), a low thump, and a quiet secondary "settle" click. Mutable in settings.
